...
class TimeseriesMetaConditionTest(APITestBase):
    ...
    def setUp(self):
        ...
        self.table_name = make_table_name(prefix)
        ...
        meta = metadata.TimeseriesTableMeta(self.table_name, metadata.TimeseriesTableOptions(172800))
    ...
    def tearDown(self):
        try:
            self.client_test.delete_timeseries_table(self.table_name)
        except Exception:
            pass
    ...
    def test_timeseries_meta_condition_api(self):
        ...
        rows = [row1, row2]
        client.put_timeseries_data(self.table_name, rows)

        # update meta
        attri = {"aaa": "bbb", "ccc": "dddd"}
        meta = metadata.TimeseriesMeta(key1, attri)
        req = metadata.UpdateTimeseriesMetaRequest(self.table_name, [meta])
        ...
        request = metadata.QueryTimeseriesMetaRequest(self.table_name, cond)
        ...
        request = metadata.QueryTimeseriesMetaRequest(self.table_name, composite_cond)
        res = client.query_timeseries_meta(request)
        self.assert_equal(len(res.timeseriesMetas), 1)
        self.assert_equal(res.timeseriesMetas[0].timeseries_key.measurement_name, key1.measurement_name)
        ...
        request = metadata.QueryTimeseriesMetaRequest(self.table_name, limit =1)
        res = client.query_timeseries_meta(request)
        self.assert_equal(len(res.timeseriesMetas), 1)
        request = metadata.QueryTimeseriesMetaRequest(self.table_name, limit=2, nextToken=res.nextToken)
        res = client.query_timeseries_meta(request)
        self.assert_equal(len(res.timeseriesMetas), 1)
